...
export class DefaultConstraintFactory implements ConstraintFactory {
  ...
  public textSize(
    ...
    return new TextSizeConstraint(
      SVGRenderer2.INSTANCE,
      paddingX,
      paddingY,
      lineHeight,
      resizeWidth,
...
export class C4Control implements Renderer {
  constructor(private readonly shapePlugin: ShapePlugin) {}
  ...
  public createDefaultShape() {
    ...
    const constraint = this.shapePlugin.constraint?.(
      DefaultConstraintFactory.INSTANCE
    );
    const configurables = this.shapePlugin.configurables?.(
      DefaultConfigurableFactory.INSTANCE
    );
  ...
  public render(
    ...
    const localRect = new Rect2(
      0,
      0,
      shape.transform.size.x,
      shape.transform.size.y
    );
