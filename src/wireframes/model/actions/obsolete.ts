...
import {addShape} from '@app/wireframes/redux/reducers/items.ts';
    ...
    createAction('items/addImage', (diagram: DiagramRef, source: string, x: number, y: number, w: number, h: number, shapeId?: string) => {
        return {
            payload: createDiagramAction(diagram, {
                shapeId: shapeId || MathHelper.nextId(),
                source,
                position: {x, y},
                size: {w, h}
            })
 ...
 * @deprecated Replaced with addShape
 */
export const addVisual = createAction(
    'items/addVisual',
    (diagram: DiagramRef, renderer: string, x: number, y: number, appearance?: object, shapeId?: string, width?: number, height?: number) => {
        return {
            payload: createDiagramAction(diagram, {
                shapeId: shapeId || MathHelper.nextId(),
                renderer,
