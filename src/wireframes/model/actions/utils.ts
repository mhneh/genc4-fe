...
export type DiagramRef = string | Diagram | undefined;
...
export function createItemsAction<T extends {}>(
  diagram: DiagramRef,
  items: ItemsRef,
  action?: T
): T & AnyAction & ItemsAction {
...
export function createRelationshipAction<T extends {}>(
  diagram: DiagramRef,
  action?: T
): T & AnyAction & RelationshipAction {
  const result: any = createDiagramAction(diagram, action);
  return result;
}

export function createDiagramAction<T extends {}>(
  diagram: DiagramRef,
  action?: T
): T & DiagramAction {
  const result: any = {};
...
export function createClassReducer<S>(
  initialState: S,
