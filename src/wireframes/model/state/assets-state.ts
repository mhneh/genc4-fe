...
export const createInitialAssetsState: () => AssetsState = () => {
    ...
    const allComponents = RendererService.all()
        .filter((x) => x[1].plugin().type() === "Components" || x[1].plugin().type() === "Modules")
        .filter((x) => x[1].plugin().showInGallery
            ? x[1].plugin().showInGallery?.() : false)
        .map(([name, renderer]) => {
            return {
                plugin: renderer.plugin(),
                displayName: name,
    ...
    const allDetails = RendererService.all()
        .filter((x) => x[1].plugin().type() === "Screens")
        .map(([name, renderer]) => {
            return {
                plugin: renderer.plugin(),
                displayName: name,
