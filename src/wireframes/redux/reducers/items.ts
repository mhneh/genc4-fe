...
export const lockItems = createAction(
    "items/lock",
...
export const unlockItems = createAction(
    "items/unlock",
    (diagram: DiagramRef, items: ItemsRef) => {
        return {payload: createItemsAction(diagram, items)};
    }
...
export const renameItems = createAction(
    "items/rename",
    (diagram: DiagramRef, items: ItemsRef, name: string) => {
        return {payload: createItemsAction(diagram, items, {name})};
    }
);

export const pasteItems = createAction(
    "items/paste",
    (diagram: DiagramRef, json: string, offset = 0) => {
        return {
            payload: createDiagramAction(diagram, {
...
export const addDesc = createAction(
    "desc/add",
...
export const removeDesc = createAction(
    "desc/remove",
...
export const updateDescs = createAction(
    "desc/updateAll",
...
export function buildItems(builder: ActionReducerMapBuilder<EditorState>) {
    return builder
        ...
        .addCase(removeItems, (state, action) => {
            ...
            return state.updateDiagram(diagramId, (diagram) => {
                ...
                for (const item of set.nested.values()) {
                    const relation = diagram.relationships.values.filter(
                        (r) => r.source == item.id || r.target == item.id
                    );
                    if (!relation) {
                        continue;
                    }
        ...
        .addCase(addShape, (state, action) => {
            ...
            let newShape: DiagramItem;
            const newState = state.updateDiagram(diagramId, (diagram) => {
                ...
                const initialProps = {
                    ...
                    transform: new Transform(
                        ...
                        new Vec2(initialSize.x, initialSize.y),
                        Rotation.ZERO
                    ),
        ...
        .addCase(updateDescs, (state, action) => {
            const {diagramId, itemIds, descs} = action.payload;
            return state.updateDiagram(diagramId, (diagram) => {
                return diagram.updateItems(itemIds, (item) => {
