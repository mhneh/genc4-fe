...
export const RenderLayer = memo((props: RenderLayerProps) => {
  const { diagram, diagramLayer, preview, onRender } = props;
  ...
  const orderedShapes = useMemo(() => {
    ...
    if (items && itemIds) {
      const handleContainer: (itemIds: ImmutableList<string>) => any = (
        itemIds
      ) => {
  ...
  useEffect(() => {
    ...
    for (const relationship of orderedRelationships) {
      if (!references[relationship.id]) {
        ...
        references[relationship.id] = new RelationshipRef(
          diagramLayer,
  ...
  useEffect(() => {
    ...
    for (const shape of allShapes) {
      if (!references[shape.id]) {
        ...
        references[shape.id] = new ShapeRef(
          diagramLayer,
          rendererInstance,
          showDebugOutlines
        );
      ...
      if (event.type === "Update") {
        if (orderedRelationships) {
          for (const shape of Object.values(event.items)) {
            ...
            const affectedRelationships = findRelationship(
            ...
            for (const relationship of affectedRelationships) {
              relationshipRefsById.current[relationship.id]?.setPreview(
                relationship,
                shape
              );
            }
...
function findDiagramItem(
  orderedShapes: DiagramItem[],
  itemId: string
