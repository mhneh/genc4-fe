/*
 * mydraft.cc
 *
 * @license
 * Copyright (c) Sebastian Stehle. All rights reserved.
 */

export const texts = {
  common: {
    about: "About",
    advanced: "Advanced",
    alignHorizontalCenter: "Horizontal Center",
    alignHorizontalLeft: "Horizontal Left",
    alignHorizontalRight: "Horizontal Right",
    alignment: "Alignment",
    alignVerticalBottom: "Vertical Bottom",
    alignVerticalCenter: "Vertical Center",
    alignVerticalTop: "Vertical Top",
    apply: "Apply",
    backgroundColor: "Background Color",
    bringForwards: "Bring Forwards",
    bringToFront: "Bring to Front",
    colors: "Colors",
    copy: "Copy",
    copyTooltip: "Copy Items",
    detailTooltip: "Used to link this to detail page.",
    describeTooltip: "Used to open modal to describe this.",
    linkTooltip: "Link to detail diagram",
    custom: "Custom",
    cut: "Cut",
    cutTooltip: "Cut Items",
    relationship: 'Relationship',
    connect: "Connect",
    emptyRel: "Please describe the relationship!",
    disConnect: "Disconnect",
    delete: "Delete",
    deleteDiagramConfirm: "Do you really want to delete the diagram?",
    title: "Title",
    desc: "Description",
    tech: "Technology",
    properties: "Properties",
    common: "Common",
    blog: "Blog",
    ecommerce: "Ecommerce",
    diagram: "Diagram",
    distributeHorizontally: "Distribute Horizontally",
    distributeVertically: "Distribute Vertically",
    duplicate: "Duplicate",
    findIcon: "Find Icon",
    findPage: "Find Page",
    findShape: "Find Shape",
    findComponent: "Find Any Component",
    fontSize: "Font Size",
    foregroundColor: "Foreground Color",
    group: "Group",
    groupTooltip: "Group Items",
    height: "Height",
    functions: "Functions",
    layout: "Layout",
    link: "Link",
    detail: "Detail",
    describe: "Describe more!",
    load: "Load",
    loadingDiagram: "Loading diagram...",
    loadingDiagramDone: "Loading diagram completed successfully.",
    loadingDiagramFailed: "Loading diagram failed with internal error.",
    loadingTriggerGenCode: "Please wait for gen code ...",
    lock: "Lock",
    lockTooltip: "Lock or unlock item",
    masterPage: "Master Page",
    more: "More",
    newDiagram: "New",
    newDiagramTooltip: "New Diagram",
    newVersion: "A new version is available. Please press F5 to reload them.",
    none: "None",
    noRecentDocument: "No recent document found",
    outline: "Outline",
    openFromFile: "Open from file",
    openFromFileTooltip: "Open diagram from file",
    optionsTooltip: "Show more options (CTRL + O)",
    ordering: "Ordering",
    page: "Page",
    pages: "Pages",
    level: "Level",
    palette: "Palette",
    paste: "Paste",
    pasteTooltip: "Paste Items",
    printDiagrams: "Print Diagrams",
    printingStarted: "Printing started...",
    recent: "Recent",
    redo: "Redo",
    remove: "Delete",
    rename: "Rename",
    removeTooltip: "Delete selected items",
    saveDiagram: "Save",
    saveDiagramToFile: "Save to File",
    saveDiagramToFileTooltip: "Save Diagram to File",
    saveDiagramTooltip: "Save Diagram",
    savingDiagram: "Saving diagram...",
    savingDiagramDone: "Saving diagram completed successfully.",
    savingDiagramFailed: "Saving diagram failed with internal error.",
    savingDiagramDoneUrl: (fullUrl: string) =>
      `Saving diagram completed under ${fullUrl}.`,
    sendBackwards: "Send Backwards",
    sendToBack: "Send to Back",
    shapes: "Shapes",
    icons: "Icons",
    contexts: "Contexts",
    containers: "Containers",
    components: "Components",
    strokeColor: "Stroke Color",
    strokeThickness: "Stroke Thickness",
    textAlignment: "Text Alignment",
    undo: "Undo",
    ungroup: "Ungroup",
    ungroupTooltip: "Ungroup Items",
    unsaved: "(unsaved)",
    unLink: "Unlink",
    visual: "Visual",
    width: "Width",
    zoomIn: "Zoom In",
    zoomOut: "Zoom Out",
    screens: "Screens",
  },
};
